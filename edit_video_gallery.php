...
<head>
<meta charset="utf-8">
<title>ویرایش گالری ویدیو</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<link rel="stylesheet" href="bootstrap/dist/css/bootstrap-rtl.min.css">
<link href="css/edit_video_gallery.css" rel="stylesheet" type="text/css">
...
function post($x){
	if(!empty($_POST[$x])){
		...
		if($_SERVER["REQUEST_METHOD"]=="POST"){
			...
			$file=((!empty($_FILES['upload'])) && ($_FILES['upload']['size'] < 99999999))?$_FILES['upload']:"";
			...
			if(!empty($file['name'])){
				...
				$address='video/'.mt_rand(1,100000).md5($file['name']).'.'.$extesion;
				move_uploaded_file($file['tmp_name'],$address);
				$topic=post("topic");
				$topic=mysqli_real_escape_string($connection,$topic);
				$connection=mysqli_connect("localhost","root","","asbdavani");
				$query="insert into videos (address,topic) values ('$address','$topic') ";
			...
			if(!empty($delete) && !empty($id)){
				...
				if(mysqli_num_rows($result)>0){
					$videoInfo=mysqli_fetch_assoc($result);				
				} // nabayd in aculad bad az unlink bashe???
				$query="delete from videos where id='$id'";
				mysqli_query($connection,$query);
				if(file_exists($videoInfo['address'])){
					unlink($videoInfo['address']);
				}	
			}	
		}
	?>
...
<form method="POST" enctype="multipart/form-data" class="form1">
    	<p class="para"> ویدیوی مورد نظر خود را انتخاب کنید ...</p>
	...
	if(mysqli_num_rows($result2)>0){
		...
		while($info=mysqli_fetch_assoc($result2)){
			...
			echo "
				...
				<form method='post'>
					<div class='col-md-4'>	
					<video width='375' height='290' class='video' controls style='display:inline;'>
					    <source src='$alladdress' type='video/mp4'>
					</video>
