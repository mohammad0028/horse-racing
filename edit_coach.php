...
<head>
<meta charset="utf-8">
<title>ویرایش اطلاعات مربی ها</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<link rel="stylesheet" href="bootstrap/dist/css/bootstrap-rtl.min.css">
<link href="css/edit_coach.css" rel="stylesheet" type="text/css">
...
<div class="div1">
<span id="enter">اطلاعات مربی ها را ویرایش کنید ...</span>
</div>
...
<table class="table1">
    ...
    <tbody id="coach" style="color:rgba(129,7,9,1.00);">
		<?php		
			function post($x){
				if(!empty($_POST[$x])){
					return $_POST[$x];
			...
			if($_SERVER["REQUEST_METHOD"]=="POST"){
				...
				$count=count($coach_name);	
				
				for($i=0;$i<$count;$i++){
					if($ids[$i]<0){
					$query1="insert into coach_info (name,race_count,firstplace_count,secondplace_count,thirdplace_count,score)
						 values ('$coach_name[$i]','$race_count[$i]','$first_place[$i]','$second_place[$i]','$third_place[$i]','$score[$i]')";
					echo 'inserted';
					}else{
					$query1="update coach_info set name='$coach_name[$i]',race_count='$race_count[$i]',firstplace_count='$first_place[$i]',secondplace_count='$second_place[$i]',thirdplace_count='$third_place[$i]',score='$score[$i]' where id='$ids[$i]' ";
					echo 'updated';
					}
				...
				if(isset($_POST['delete']) ){
					...
					$query="select * from coach_info where id='$delete'";
					$result=mysqli_query($connection,$query);
					if(mysqli_num_rows($result)>0){
					$query="delete from coach_info where id='$delete'";
			...
			if(mysqli_num_rows($result)>0){
				while($data=mysqli_fetch_assoc($result)){
					...
					echo '
						<tr>
							<td>'.$i.'</td>
							<td><input style="background-color:transparent;" class="input1" type="text" name="coach_name[]" value="'.htmlentities($data['name']).'" disabled>
							</td>
...
<button onclick="addCoach()" type="button" class="horse" title="افزودن"><span class="glyphicon glyphicon-plus" id="glyph"></span></button>
...
<script>
	...
	function addCoach(){
		var rowNum=document.querySelectorAll('#coach tr').length;
		rowNum++;
		$("#coach").append("<tr><td style='color:white;'>"+rowNum+"</td>"
											+"<td><input class='input1' name='coach_name[]' type='text'></td>"
											+"<td><input class='input2' name='race_count[]' type='text'></td>"
											+"<td><input class='input3' name='first_place[]' type='text'></td>"
											+"<td><input class='input4' name='second_place[]' type='text'></td>"
											+"<td><input class='input5' name='third_place[]' type='text'></td>"
											+"<td><input class='input6' name='score[]' type='text'></td>"
											+"<td>"
